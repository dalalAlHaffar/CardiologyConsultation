    ...
    <div class="col-md-9">
      <div class="card">
        <div class="card-body">
          ...
          <form action="{{url('cms/blog/').'/'.$blog->id}}" method="POST" enctype='multipart/form-data' class="needsValidation">
            @csrf
            @method('PUT')
            <x-input>
              ...
              <x-slot name="value">{{$blog->title}}</x-slot>
            </x-input>
            <x-input>
              ...
              <x-slot name="forLogin">no</x-slot>
              <x-slot name="value">{{$blog->description}}</x-slot>
            </x-input>
            <x-input>
              ...
              <x-slot name="forLogin">no</x-slot>
              <x-slot name="value">{{$blog->brief_description}}</x-slot>
            </x-input>
            <x-input>
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-10">
                <select class="form-select select2 @error('category_id') laravel-invalied @enderror" placeholder="bla" name="category_id" aria-label="Default select example">
                  ...
                  <option value="{{$blog->category->id}}">{{$blog->category->title}}</option>
                  @endif
                </select>
                @error('category_id')
