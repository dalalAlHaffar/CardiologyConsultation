    ...
    <div class="col-md-9">
      <div class="card">
        <div class="card-body">
          ...
          <form action="{{url('cms/user').'/'.$user->id}}" method="POST" enctype='multipart/form-data' class="needsValidation">
            @csrf
            @method('PUT')
            <x-input>
              ...
              <x-slot name="value">{{$user->name}}</x-slot>
            </x-input>
            <x-input>
              ...
              <x-slot name="forLogin">no</x-slot>
              <x-slot name="value">{{$user->email}}</x-slot>
